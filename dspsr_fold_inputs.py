...
def on_message(body,opts):
    ...
    with open('%s.txt'%(opts.output_file_path+folding_packet['source']),'w') as f:
    ...
    print "dspsr -P %s.txt %s" %(folding_packet['source'],folding_packet['file path'])
    subprocess.check_call(["dspsr","-P",opts.output_file_path+folding_packet['source']+".txt",folding_packet['file path'],"-O",opts.output_file_path+folding_packet['source']])
